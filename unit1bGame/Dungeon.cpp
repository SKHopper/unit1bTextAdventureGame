...
vector<DoorWall> Dungeon::getAdjacentSides(bivarInt coordinate){ //TODO MAYBE: change to pass by ref "fill with" over "get"
    ...
    for (int i = 0; i < 4; i++) {
        ...
        int roomIndex = getRoomIndex(sumBivariateIntegers(coordinate, getDirectionDisplacement(direction)));
        if (roomIndex != -1) {
            DoorWall& RtwinSide = roomMap.at(roomIndex).room.getSide(constants::DIRECTION_OPPOSITE.at(direction));
